...
def create_multi_csv_bar_chart_arxiv(
    ...
    for i, (ax, csv_file, x_labels, subplot_title) in enumerate(
        ...
        for bars in bars_group:
            for rect in bars:
                ...
                if val > 2.0:
                    ...
                    ax.text(x_pos + 0.75*width, 1.6, str(round(val, 1)),
                            fontsize=xlabel_font*0.8, color="black")
                    ax.annotate("",
                        xy=(x_pos + 0.6*width, 2.0),
                        xytext=(x_pos + 1.0*width, 1.9),
        ...
        ax.axhline(y=1.5, color='gray', linestyle='-', linewidth=2, alpha=0.7, zorder=1)
    ...
    fig.legend(
        handles,
        labels,
        bbox_to_anchor=[axbox.x0 + 1.05 * axbox.width + 0.59, axbox.y0 + axbox.height + 0.375],
        ncol=3,
...
# Save
# fig.savefig("./dynamic_par/figure11_new.pdf", dpi=300, bbox_inches="tight")
fig.savefig("./dynamic_par/figure21.pdf", bbox_inches="tight")
