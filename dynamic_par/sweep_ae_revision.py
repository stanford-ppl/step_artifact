...
from dynamic_par.static_coarse_parallel import (
    run_static_coarse_par,run_static_coarse_par_batch_sweep
)
from dynamic_par.static_parallel import run_static_par
from dynamic_par.dynamic_parallel import run_dynmic_par as run_dynamic_par
...
def test_b64_sweep():
    ...
    for kv_length_var, batch_list in batch_list_b64.items():
        ...
        for batch_spec in batch_list:
            ...
            raw_result_dict["start"] = batch_spec["start"]
            raw_result_dict["end"] = batch_spec["end"]
            ...
            dynamic_cycles = run_dynamic_par(
                batch=batch,
                cache_row_offset_tiled=cache_row_offset_tiled,
                tile_N=tile_N,
                metadata_fifo_depth=metadata_fifo_depth,
    ...
    # save results to csv
    out_file = f"./dynamic_par/batch64_interleave_dynamic.csv"
    try:
...
def test_batch_sweep():
    ...
    # ---- config for batch 64 ----
    batch_spec= {"start": 4007, "end": 4070, "stdev": 996} # 70209, 64361, 49093
    check_intermediate = False
    ...
    for batch in [16,32,48,64]:
        ...
        raw_result_dict = {}
        raw_result_dict["batch"] = batch

        trace_file = f"./dynamic_par/azure_trace/b64/conv_stdev{batch_spec['stdev']:04d}_{batch_spec['start']:04d}_{batch_spec['end']:04d}.npy"
        raw_np_arr = np.load(trace_file)
        np_arr_int = raw_np_arr.astype(np.int64)
        num_token_list = np_arr_int.tolist()*3
        num_token_list = num_token_list[:batch]
        print(f"num_token_list: {num_token_list}")
        ...
        static_coarse_cycles = run_static_coarse_par_batch_sweep(
            batch=batch,
            cache_row_offset_tiled=cache_row_offset_tiled,
            tile_N=tile_N,
            metadata_fifo_depth=metadata_fifo_depth,
        ...
        if check_intermediate:
            with open(
                f"./dynamic_par/static_coarse_batch_sweep_stdev{batch_spec['stdev']:04d}_{batch_spec['start']:04d}_{batch_spec['end']:04d}.txt",
                "a",
                encoding="utf-8",
            ) as file:
    ...
    # save results to csv
    out_file = f"./dynamic_par/batch_sweep_coarse_vs_dynamic.csv"
    try:
        with open(out_file, "w", newline="", encoding="utf-8") as csvfile:
            fieldnames = [
                "kv_length_var",
