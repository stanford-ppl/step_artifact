...
def call_ws_tile_mn_mk_gemm_reshape_revet(
    ...
    output, unit_expert_on_chip, allocated_comp_flops = (
        ws_tile_mn_mk_gemm_reshape_revet(
...
def run_ws_tile_mn_mk_revet(
    ...
    output, off_chip_traffic, on_chip_requirement, cycles, duration_s, unit_expert_on_chip, allocated_comp_flops = call_ws_tile_mn_mk_gemm_reshape_revet(  # type: ignore (Cannot infer type of output properly)
        model_config=model_config,
        batch=B,
        gate_compute_bw=GATE_COMPUTE_BW,
        up_compute_bw=UP_COMPUTE_BW,
