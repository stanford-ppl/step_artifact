...
def docker_copy(docker_id, src_path, dst_path):
    ...
    """
    cmd = ["docker", "cp", f"{docker_id}:{src_path}", dst_path]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--docker_id", required=True, help="Container name or ID (e.g., step_run)"
    )
    parser.add_argument(
        "--output_dir", required=True, help="Local directory to copy files into"
    )
    parser.add_argument("--copy_log", required=False, help="whether to copy log")
